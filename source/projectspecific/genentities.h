void genentities(){
	...
	for(int i=0;i<20;i++){
		if (i>=0 && i<4){
		worldgrid[door_array[i][0]][door_array[i][1]][door_array[i][2]][0]=door_tile;
		}
		else
		{
			worldgrid[door_array[i][0]][door_array[i][1]][door_array[i][2]][0]= interior_door_tile;
			worldgrid[door_array[i][0]][door_array[i][1]][door_array[i][2]][1]=door_array[i][3];
		}
	...
	for(int x=0; x<worldgridsizex; x++)
		for(int y=0; y<worldgridsizey; y++)
			for(int z=0; z<worldgridsizez; z++)
				if(worldgrid[x][y][z][0]==door_tile)
				{
					    door[door_tmp].type=1; 
						door[door_tmp].xpos=(door_array[door_tmp][0]*10);  
						door[door_tmp].ypos=(door_array[door_tmp][1]*10);
						door[door_tmp].zpos=(door_array[door_tmp][2]*10); 
						door_tmp++; 
				}
				else if(worldgrid[x][y][z][0]==interior_door_tile)
				{
					 door[door_tmp].type=1; 
				     door[door_tmp].xpos=(door_array[door_tmp][0]*10);  
					 door[door_tmp].ypos=(door_array[door_tmp][1]*10);
